...
private products: Product[];
  constructor() { 
    this.products = [{id :UUID.UUID(),name:"computer",price:6500,promotion:true},
  ...
  public getPageProducts(page:number,size:number) : Observable<PageProduct>{
    ...
    let pagesProduct = this.products.slice(index,index + size)
    return of ({page : page, size : size, totalPages:totalPages,products: pagesProduct})
  }
...
public handleSetPromo(id : string) :Observable<boolean> {
...
public searchProduct(keyword : string , page:number, size:number) : Observable<PageProduct> {
 ...
 let index = page*size;
    ...
    let pagesProduct = result.slice(index,index + size)
    return of ({page : page, size : size, totalPages:totalPages,products: pagesProduct})
}
}
